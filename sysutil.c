...
const char* statbuf_get_date(struct stat* sbuf)
{
		...
		strftime(datebuf, sizeof(datebuf), p_date_format, p_tm);//ri qi geshihua
		return datebuf;
}
...
static int lock_internal(int fd, int lock_type)
{
	...
	the_lock.l_type = lock_type;//suo de lei xin
	the_lock.l_whence = SEEK_SET;// cong nali kaishi jia suo
	the_lock.l_start = 0;// suo de pian yi weizhi,cong wenjian tou kaishi
	the_lock.l_len = 0;//jiang zhengge wen jian jiasuo
	
	do
	{
		//chuli jiasuo shi zhongduan
		ret = fcntl(fd, F_SETLKW, &the_lock);
	}
	while(ret < 0 && errno == EINTR);

	return ret;
	//ret==0 biaoshi jiasuo chenggong
...
int unlock_file(int fd)
{
	...
	struct flock the_lock;
	memset(&the_lock, 0, sizeof(the_lock));
	the_lock.l_type = F_UNLCK;//suo de lei xin
	the_lock.l_whence = SEEK_SET;// cong nali kaishi jia suo
	the_lock.l_start = 0;// suo de pian yi weizhi,cong wenjian tou kaishi
	the_lock.l_len = 0;//jiang zhengge wen jian jiasuo
		//chuli jiasuo shi zhongduan
...
void nano_sleep(double seconds)
{
	...
	while(ret == -1 && errno == EINTR);//ruguo bei xinhao zhongduan le,nahuanshi diao yong shui mian hanshu,chuandi jin lai de shijian jiushi shengyu de shijian,ts canshu hui bei fanhui fugei ts,zhidao shui mian shijian goule
}


void activate_oobinline(int fd)
{
	int oob_inline = 1;//1 buiaoshi kaiqi jinji jieshou moshi
	int ret;
